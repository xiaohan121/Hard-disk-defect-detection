...
class FRCNNDataset(Dataset):
    ...
    def get_random_data(self, annotation_line, input_shape, jitter=.3, hue=.1, sat=0.7, val=0.4, random=True):
        ...
        for i in range(label.shape[0]):
            box[i, 0] = int(label[i, 1] - (label[i, 3] / 2))
            box[i, 2] = int(label[i, 1] + (label[i, 3] / 2))
            box[i, 1] = int(label[i, 2] - (label[i, 4] / 2))
            box[i, 3] = int(label[i, 2] + (label[i, 4] / 2))
            box[i, 4] = label[i, 0] - 1
